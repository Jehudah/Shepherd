...
export default function ThemesLessons() {
  ...
  const theme = expandedThemes.find(t => t.id === themeId);

  if (!theme) {
    return <div>Theme not found</div>;
  }

  // Create 5 lessons with progressive difficulty
  const lessons = [
    { id: `${theme.id}-1`, title: `${theme.name}: Foundations`, difficulty: 1, questions: 12 },
    { id: `${theme.id}-2`, title: `${theme.name}: Key Verses`, difficulty: 2, questions: 12 },
    { id: `${theme.id}-3`, title: `${theme.name}: Applications`, difficulty: 3, questions: 12 },
    { id: `${theme.id}-4`, title: `${theme.name}: Deep Dive`, difficulty: 4, questions: 12 },
    { id: `${theme.id}-5`, title: `${theme.name}: Master Challenge`, difficulty: 5, questions: 12 }
  ];
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {/* Header */}
      <div className="text-white rounded-3xl p-6 sm:p-8 shadow-xl" style={{ background: `linear-gradient(to right, ${theme.color}, ${theme.color}dd)` }}>
            ...
            >
              ...
              {canAccess ? (
                <div className="space-y-2">
                  <Link
                    to={`/lesson/themes/${themeId}/${lesson.id}`}
                    className={`w-full ${
                      isCompleted
